...
function DefaultTableHead(props) {
  const {
    classes,
    order,
    orderBy,
    ...
    <TableHead>
      <TableRow>
        ...
        {columns.map(headCell => (
          <TableCell
            key={headCell.id}
            align={headCell.numeric || headCell.action ? 'right' : 'left'}
            padding={'default'}
            sortDirection={orderBy === headCell.id ? order : false}
            size={headCell.action ? 'small' : 'medium'}
            style={headCell.action ? { maxWidth: '130px', width: '130px' } : {}}
          >
            <TableSortLabel
...
const DefaultTableToolbar = props => {
  ...
  const { numSelected, title, actionAdd } = props;
        ...
        <>
          {actionAdd && (
            <Tooltip title="Adicionar">
              <IconButton aria-label="adicionar" onClick={actionAdd}>
                <AddIcon />
              </IconButton>
            </Tooltip>
          )}
...
function DefaultTable(props) {
  const { title, actionAdd, actionEdit, actionDelete, array, columns } = props;
  ...
  const emptyRows =
    rowsPerPage - Math.min(rowsPerPage, array.length - page * rowsPerPage);
        ...
        <TableContainer>
          ...
          >
            <DefaultTableHead
              ...
              columns={columns}
              showActions={actionEdit || actionDelete}
            />
            <TableBody>
                ...
                .map((row, index) => {
                  ...
                  return (
                    <TableRow hover role="checkbox" tabIndex={-1} key={labelId}>
                      {columns.map((c, i) => {
                        // treta para acessar obj por 'Loja.nome' direto no id
                        const arrayProp = c.id.split('.');
                        let value = row;
                        arrayProp.map(s =>
